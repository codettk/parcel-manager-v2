...
export interface UiState {
  ...
  /**
   * 영농 ERP 업무일지 뷰 열림 (슬라이스 5b) — 인력 뷰(5a) 동형 풀스크린 레이어.
   * 작성·수정 시트는 뷰 내부 로컬 상태(draft 패턴)라 전역은 뷰 열림만 (CONVENTIONS §3).
   * PRO 게이팅 강제 없음 — 진입점만 NavDrawer PRO 섹션에 둔다(슬라이스 6 자리, AC-13).
   */
  workLogViewOpen: boolean
