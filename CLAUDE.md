...
- **지도 렌더**: `src/features/map/engine/`은 React를 import하지 않는 순수 TS Canvas 2D 렌더 엔진 (M-2 완료 — scene·renderScene·outerEdges memo·viewport·hitTest, ESLint로 react import 금지 강제). `MapCanvas.tsx`는 캔버스/DPR/ResizeObserver 관리와 엔진 호출만 하는 얇은 React 호스트. 렌더 패스는 v1 보존 8개(1차~7차 + 1.5차 색없는그룹) + 별도 라벨 캔버스(M-4 예정). v1 보존 색상 상수는 `engine/colors.ts`(lint hex 예외 모듈)에 집약.
